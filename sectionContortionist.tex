...
how to make the scattered bits of sound in the corpus into a smooth map through which the player can navigate (section~\ref{sec:mix}). A further sonic problem is then tackled in (section~\ref{sec:preemph}).
...
As the direct signal from the piezo is used to convolve the grains, a preprocessing of its sonic caracteristics has been required: indeed the microphone will exhibit some grain of the surface on which it is applied, most of the time resulting in a dull top-end, with overdynamic spikes when getting too near the microphone. Moreover, the piezo's high impedence and low gain usually requires high pre-amplifiyer gains, which is prone to generate background noise. We have found that a three band filtering (low high-pass, a band-cut on the main resonance and a boosting high-shelf) followed by a subtle low-threshold expander and a high-threshold limiter are pre-conditionning the signal and yield results that are much nearer the timbral caracteristics of the IRs selected.